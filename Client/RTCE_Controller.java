...
public class RTCE_Controller implements ActionListener {
        ...
        SwingWorker<?,?> worker = new SwingWorker<String, Void>() {
            ...
            public void done() {
                try {
                     ...
                     * 
                     * Furthermore, reassign caret position by comparing the number of characters following
                     * the caret index before and after the edit. Logic is as follows:
                     * 
                     * (1) If the number of characters after the caret has increased, there has been an insertion.
                     *     (a) If the substring preceding the caret is unchanged, the insertion took place after
                     *         the caret and it should retain its original position.
                     *     (b) Otherwise, the insertion took place before the caret, which is shifted by the length
                     *         of said insertion.
                     * (2) If the number of characters after the caret has decreased, there has been a deletion.
                     *     (a) If the substring succeeding the caret is unchanged, the deletion took place after
                     *         the caret and it should retain its original position.
                     *     (b) If characters are deleted from both sides of the caret, shift the caret to the
                     *         beginning of the deletion (calculated via the findDivIndex helper function).
                     *     (c) If characters are only deleted before the caret, shift the caret by the length
                     *         of said deletion.
                     * (3) If the number of characters after the caret are the same, retain original position.
                     */
                    if(controllerEvent.getSource() == viewTimer) {
                        String viewResponse = get();
                        int index = view.getTextPane().getCaretPosition(); // caret index
                        int oldDiff = view.getTextPane().getText().length() - view.getTextPane().getCaretPosition();

                        String preStringIns = view.getTextPane().getText().substring(0, index+1);
                        String preStringDel = view.getTextPane().getText().substring(0, index);
                        String postStringDel = view.getTextPane().getText().substring(index, view.getTextPane().getText().length());

                        model.updateText(viewResponse);
                        int newDiff = view.getTextPane().getText().length() - index;
                        if (newDiff > oldDiff) { // insertion detected
                            if ((view.getTextPane().getText().substring(0, index+1)).equals(preStringIns))
                                view.getTextPane().setCaretPosition(index); // no change
                            else {
                                view.getTextPane().setCaretPosition(index + (newDiff - oldDiff)); // translate
                            }
                        } else if (newDiff < oldDiff) { // deletion detected
                            if ((view.getTextPane().getText().substring(0, index)).equals(preStringDel))
                                view.getTextPane().setCaretPosition(index); // no change
                            else {  // translate
                                if (!view.getTextPane().getText().substring(index, view.getTextPane().getText().length()).equals(postStringDel)) {
                                    view.getTextPane().setCaretPosition(Math.max(0, findDivIndex(preStringDel, view.getTextPane().getText().substring(0, index))));
                                } else
                                    view.getTextPane().setCaretPosition(Math.max(0, index + (newDiff - oldDiff)));
                            }
                        } else if (newDiff == oldDiff) // no edit
                            view.getTextPane().setCaretPosition(index);
                    }
                    /*
    ...
    /**
     * Helper function. Given two strings, return the index at which they first start to
     * diverge from one another. Used to assign caret position in the case of a delete
     * operation in which characters are deleted from both sides of the caret.
     * 
     * @param s1 The first string.
     * @param s2 The second string.
     * @return The index at which they first differ. Returns the length of the shorter string
     * if it happens to be a substring of the longer string.
     */
    public int findDivIndex(String s1, String s2) {
        ...
        return Math.min(c1.length, c2.length);
    }
    ...
    public  void handleInsertUpdate(final int pos, final String text) {
        SwingWorker<?,?> worker = new SwingWorker<String, Void>() {
            ...
            /* Update view for an insert request. Furthermore, reassign caret position by
             * comparing the number of characters following the caret index before and after
             * the edit. See viewTimer done() method for full description of the various
             * scenarios.
             */
            @Override
            public void done() {
                ...
                try {
                    insertReq = get();
                    int index = view.getTextPane().getCaretPosition();
                    int oldDiff = view.getTextPane().getText().length() - view.getTextPane().getCaretPosition();
    ...
    public  void handleDeleteUpdate(final int startLocation, final int endLocation) {
        SwingWorker<?,?> worker = new SwingWorker<String, Void>(){
            ...
            /* Update view for a delete request. Furthermore, reassign caret position by
             * comparing the number of characters following the caret index before and after
             * the edit. See viewTimer done() method for full description of the various
             * scenarios.
             */
            ...
            public void done() {
                ...
                try {
                    ...
                    String preStringIns = view.getTextPane().getText().substring(0, index);
                    String preStringDel = view.getTextPane().getText().substring(0, index);
                    String postStringDel = view.getTextPane().getText().substring(index, view.getTextPane().getText().length());
                    try {
